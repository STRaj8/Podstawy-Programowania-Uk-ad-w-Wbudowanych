...
#define VIC_TIMER0_CHANNEL_NR 4 //Vic Channel

// VICVectCntlx Vector Control Registers
#define mIRQ_SLOT_ENABLE 0x00000020 // 10 0000

void (*ptrTimer0InterruptFunction)(void);

/**********************************************/
//(Interrupt Service Routine) of Timer 0 interrupt
__irq void TIMER0IRQHandler(){				//__irq uzywa sie tego przy przewaniach, jak funckja jest wywwowlana zapisuje rejestry mowisz kompliatorowi ze to przerwanie

	T0IR=mMR0_INTERRUPT; 	// skasowanie flagi przerwania 
	ptrTimer0InterruptFunction();		// cos do roboty
	VICVectAddr=0x00; 	// potwierdzenie wykonania procedury obslugi przerwania // wpisuje sie 4 z timera
}
/**********************************************/
void Timer0Interrupts_Init(unsigned int uiPeriod, void (*ptrInterruptFunction)()){ // microseconds
	...
	VICVectAddr1  =(unsigned long)TIMER0IRQHandler; 	   // Set to Slot 0 Address of Interrupt Service Routine nazwa funckji bez nawiasow to adress funckji 
